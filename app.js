// Register EPSG:31984 (SIRGAS 2000 / UTM zone 24S) — the projection the GeoJSON points are stored in.
// Needed once, at read time, so OpenLayers can reproject them into the map's projection.
proj4.defs("EPSG:31984", "+proj=utm +zone=24 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs");
ol.proj.proj4.register(proj4);

// Base directory for the Cloud Optimized GeoTIFFs (already in EPSG:3857, no reprojection needed)
const COG_DIR = '/obl/obl_cog';
...
// Global state
let currentImageLayers = {}; // dir -> ol.layer.WebGLTile for the current click
...
// Load the image name -> subfolder map, so image URLs can be built per entrega.
...
function processClick() {
    ...
    const tasks = ALL_DIRS.flatMap(dir => {
        ...
        return f ? [loadCogLayer(f.get('name'), dir, dir === activeDir)] : [];
    });
...
function loadCogLayer(name, dir, isActive) {
    // The COG carries its own georeferencing (EPSG:3857), so OpenLayers needs no
    // world file, no image-dimension probe, and no extent math.
    const base = nameToFolder[name] ? `${COG_DIR}/${nameToFolder[name]}` : COG_DIR;
    const source = new ol.source.GeoTIFF({
        convertToRGB: true,
        sources: [{ url: `${base}/${name}.tif` }],
        interpolate: true
    });
    ...
    // Resolves once the COG metadata is read (or rejects on failure)
    return source.getView().catch(err => {
        console.error('Error loading COG for ' + name, err);
    });
}
